...
public class Stream_02_Test {
    ...
    public void test_map() throws Exception {
        ...
        List<Customer> result = orders.stream().map(m -> m.getCustomer()).distinct().collect(Collectors.toList());
    ...
    public void test_flatmap() throws Exception {
        ...
        IntSummaryStatistics result = orders.stream().flatMap(m -> m.getPizzas().stream())
        		.collect(Collectors.summarizingInt(m -> m.getPrice()));
